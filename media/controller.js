...
function tableController($scope, $http, $location){
  ...
  $scope.Headers = {"Reconstruction":
      [{name:"Release Name", db_name:"Release_Name"}, {name:"Tracker", db_name:"TK"}, {name:"Ecal", db_name:"Ecal"}, {name:"HGcal", db_name:"HGCAL"}, {name:"Hcal", db_name:"Hcal"}, {name:"CASTOR", db_name:"CASTOR"}, {name:"DT",db_name:"DT"}, {name:"CSC", db_name:"CSC"}, 
       {name:"RPC", db_name:"RPC"}, {name:"GEM", db_name:"GEM"}, {name:"L1", db_name:"L1"}, {name:"Tracking", db_name:"Tracking"}, {name:"Electron", db_name:"Electron"}, {name:"Photon", db_name:"Photon"}, {name:"Muon", db_name:"Muon"}, 
...
//       $scope.shortInfoReleases = _.uniq($scope.shortInfoReleases); //
  ...
  $scope.$watch("menus", function(v){
    ...
    _.each($scope.menus, function(single_menu){
      if (single_menu.name == "HIN"){
        correct_name = "IN"; //just because submenus in HIN/{Data} -> HData which is already for HLT
      }else{
        correct_name = single_menu.name
      }
      if(single_menu.showSubCat){
        $location.search(correct_name,'true');
      }else{
        $location.search(correct_name,null);
      }
      _.each(single_menu.subcats, function(subcat){
        if (subcat.show){
          $location.search(""+correct_name[0]+subcat.name.substring(0,4)+"",'true');
        }else{
          $location.search(""+correct_name[0]+subcat.name.substring(0,4)+"",null);
        }
      });
    });
  },true);
...
app.directive("addRelease", function($http){
  ...
  template: '<div>'+
      ...
      '        </label>'+
      '        <label class="checkbox">'+
      '          <input type="checkbox" ng-model="cat.IN">HIN</input>'+
      '        </label>'+
      '        <label class="checkbox">'+
      '          <input type="checkbox" ng-model="cat.GEN">GEN</input>'+
      '        </label>'+
      '      </div>'+
      '    </div>'+
      '    <div class="control-group">'+
      ...
      '    </div>'+
      '    <div class="control-group">'+
      '      <label class="control-label">Show optional inputs</label>'+
      '      <div class="controls">'+
      '        <input type="checkbox" ng-model="visible_options">'+
      '      </div>'+
      '    </div>'+
      '    <div class="control-group" ng-show="visible_options">'+
      '      <label class="control-label">RelMon URL:</label>'+
      '      <div class="controls">'+
      '        <input ng-class="input-xlarge" type="text" ng-model="relmon_url"></input>'+
      '      </div>'+
      '      <label class="control-label">Info column <b>text</b>:</label>'+
      '      <div class="controls">'+
      '        <textarea ng-class="input-xlarge" type="text" ng-model="info_text"></textarea>'+
      '      </div>'+
      '      <label class="control-label">Info column <b>links</b>:</label>'+
      '      <div class="controls">'+
      '        <textarea ng-class="input-xlarge" type="text" ng-model="links_url"></textarea>'+
      '      </div>'+
      '    </div>'+
      '</div> <!--end of modal body-->'+
      '<div class="modal-footer">'+
  ...
  link: function (scope, element, attr){
    ...
    scope.addRelease = function(){
      var subcats = scope.cat.subcat; //we delete subcats object because
      delete(scope.cat.subcat);       //not to include "subcat" in categories
      if (scope.cat["GEN"] == true)
      {
        subcats["Gen"] = true;
      }

      scope.pendingHTTP = true;
      $http({method:'POST', url:'addNewReleaseUpdated', data: {"categories":_.keys(scope.cat),"subcats":_.keys(subcats),"release_name":scope.release_name,"relmon_url":scope.relmon_url, "links_url":scope.links_url, "info_text":scope.info_text}})
      .success(function(data,status){
    ...
    scope.$watch("cat", function(){
      ...
      _.each(scope.cat, function(value,key){
        ...
        if (scope.cat["GEN"] == true)
        {                     //if we want only GEN with Gen Subcat
          subselect = true;   //we force subcats selections to be able to procceed
        }
        if(value){
            mainselect = true;
        }else{
...
app.directive("addUser", function($http){
  return{
    ...
    template:'<div>'+
      ...
      '            <b ng-switch-when="IN">{{"H"+category}}/{{subcat.name}}</b>'+
      '            <b ng-switch-default>{{category}}/{{subcat.name}}</b>'+
      '          </div>'+
    ...
    link: function(scope,element,attr){
      ...
      scope.user['IN'] = {};
      scope.user['IN']["Data"] = {};
      scope.user['IN']["FullSim"] = {};
      scope.user['IN']["FastSim"] = {};
      scope.user['GEN'] = {};
      scope.user['GEN']["Gen"] = {};
      ...
      scope.usrIDataStatsChecked = [];
      scope.usrIFastStatsChecked = [];
      scope.usrIFullStatsChecked = [];
      scope.usrGGenStatsChecked = [];
      ...
      scope.getHINColumns = function(){
        scope.usrIDataStatsChecked = [];
        scope.usrIFastStatsChecked = [];
        scope.usrIFullStatsChecked = [];
      _.each(scope.user['IN'], function(value,key){
        switch(key){
          case "Data":
            _.each(value, function(column, to_add){
              if(to_add){
                scope.usrIDataStatsChecked.push(to_add.toUpperCase());
              }
            });
            break;
          case "FullSim":
            _.each(value, function(column, to_add){
              if(to_add){
                scope.usrIFullStatsChecked.push(to_add.toUpperCase());
              }
              });
            break
          case "FastSim":
            _.each(value, function(column, to_add){
              if(to_add){
                scope.usrIFastStatsChecked.push(to_add.toUpperCase());
              }
            });
            break;
        }
      ...
      scope.getGENColumns = function(){
        scope.usrGGenStatsChecked = [];
      _.each(scope.user['GEN'], function(value,key){
        switch(key){
          case "Gen":
            _.each(value, function(column, to_add){
              if (to_add) {
                scope.usrGGenStatsChecked.push(to_add.toUpperCase());
              }
            });
            break;
        }
      ...
      $http({method:'POST', url:'addNewUser', 
          data: {
            ...
            usrIDataStatList: scope.usrIDataStatsChecked,
            usrIFastStatList: scope.usrIFullStatsChecked,
            usrIFullStatList: scope.usrIFullStatsChecked,
            usrGGenStatList: scope.usrGGenStatsChecked
          }
        })
        .success(function(data,status){
          scope.pendingHTTP = false;
