...
def getUsername():
    return cherrypy.request.headers.get('Adfs-Login', '???')

def getFullName():
    return cherrypy.request.headers.get('Adfs-Fullname', '???')

def getGroups():
    return cherrypy.request.headers.get('Adfs-Group', 'cms-CERN-users').split(';')
